...
The proof has to be send from the spend chain to the debit chain, which is costly and slow, as the processor has to use cross-chain bridges.
...
Only when the transaction is challenged, the validity of the transaction has to be proven, the validity of the transaction is checked, which is slow and costs more gas.
