...
* **Safenet Smart Contracts**: These contracts are deployed on every supported chain, with the Ethereum mainnet as the primary hub for configuration and coordination.
...
* **External Bridges**: External bridges are only used for dispute resolution when a validator challenges a Safenet transaction.
