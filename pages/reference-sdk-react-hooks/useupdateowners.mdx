...
<Tabs items={['App.tsx', 'main.tsx']}>
  ...
  function App() {
    const { 
      add: { addOwner, ...add },
      remove: { removeOwner, ...remove },
      swap: { swapOwner, ...swap }
    } = useUpdateOwners();

    const handleAddOwner = () => {
      addOwner({ ownerAddress: '0xAddOwnerAddress' });
    };

    const handleRemoveOwner = () => {
      removeOwner({ ownerAddress: '0xRemoveOwnerAddress' });
    };

    const handleSwapOwner = () => {
      swapOwner({ oldOwnerAddress: '0xOldOwnerAddress', newOwnerAddress: '0xNewOwnerAddress' });
    };
