...
1. Clone the [safe-smart-account](https://github.com/safe-global/safe-smart-account) repository, by running the following command:
    ...
    ```
2. If [Infura](https://www.infura.io/) supports your network, configure the following environment variables in a `.env` file:
...
3. Get the latest version of [@safe-global/safe-singleton-factory](https://www.npmjs.com/package/@safe-global/safe-singleton-factory), by running the following command:

    ```
    npm i --save-dev @safe-global/safe-singleton-factory
    ```

    Make sure the latest version includes your `safe-singleton-factory` deployment from the previous section.

4. If Infura supports your network, run the following command: 
    ```
