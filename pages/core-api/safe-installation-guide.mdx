...
Your pull request should follow this [example pull request](https://github.com/safe-global/safe-deployments/pull/679).
...
You have three options for deploying Safe\{Stack\}, depending on your needs.
...
This command runs 17 Docker containers from the [docker-compose.yml](https://github.com/gjeanmart/safe-infrastructure/blob/main/docker-compose.yml):
...
- **Frontend endpoint returns `502 Bad Gateway`**
    NextJS can take up to 5-10 minutes to generate all the pages and check the UI logs to ensure the process finished successfully.
