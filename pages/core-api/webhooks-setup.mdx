...
<Callout type="info">
  Webhooks are a paid feature. Visit [Safe API plans ↗](https://safe.global/api) for details.
</Callout>
...
1. A Safe Infrastructure account on a paid plan.
...
- **Event type**: Subscribe to specific event types only. See [Supported event types](./webhooks-overview#supported-event-types) for the full list.
...
The number of webhook subscriptions you can configure depends on your plan. Contact [support@safe.global](mailto:support@safe.global) if you need additional subscriptions beyond your plan's limit.
