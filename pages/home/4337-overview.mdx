...
``` mermaid
---
title: Simplified 4337 flow
---
flowchart TD
    A(Dapp
    Users kick off a 4337 flow through a dapp) -->|UserOperation| B(Bundler
    Decentralized network of relayers to propagate user operations)
...
>
  ...
  <Grid item xs={12} md={4}>
    <CustomCard
      ...
      description={'It is supported by various providers, avoiding lock-in to a single-relayer technology, offering an anti-fragile approach with no single point of failure.'}
      url=""
      newTab={false}
    />
  </Grid>
