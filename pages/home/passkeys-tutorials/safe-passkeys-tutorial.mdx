...
Add the following code the the `PasskeyList.tsx` file:
...
At this stage, the app will have created a safe object awaiting deployment. Send some USDC to your future safe by clicking the link to Circle's USDC faucet for Sepolia and entering the Safe's address. By clicking **Sign transaction with passkey**, the deployment of this safe will then be embedded in a batch transaction, along with the transfer of USDC. You can open the console and check the progress of your transaction and the state of the User Op via the provided link to Jiffy scan.
