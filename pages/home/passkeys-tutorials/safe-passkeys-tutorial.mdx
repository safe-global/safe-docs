...
<Callout type='warning' emoji='⚠️'>
  Passkeys support is still under audit and should not be used in production environments. Please consider this tutorial and all the code examples in it a developer preview. Compatibility with passkeys is only available when using `@safe-global/protocol-kit@4.1.0-alpha.1` and `@safe-global/relay-kit@3.1.0-alpha.1` in Sepolia.
</Callout>
...
**Prerequisite knowledge:** You will need some basic experience with [React](https://react.dev/learn), Next.js, and [ERC-4337](../erc-4337/overview.mdx).
...
For this project, we'll use the [Relay Kit](../../sdk/relay-kit.mdx) and [Protocol Kit](../../sdk/protocol-kit.mdx) from the Safe\{Core\} SDK to set up a Safe, sponsor a transaction, and use [viem](https://www.npmjs.com/package/viem).

Run the following command to add all these dependencies to the project:

```bash
pnpm add @safe-global/protocol-kit@4.1.0-alpha.1 @safe-global/relay-kit@3.1.0-alpha.1 viem
```
...
At this stage, the app will have created a safe object awaiting deployment. By clicking **Sign transaction with passkey**, the deployment of this Safe will then be embedded in a batch transaction, along with the minting of the NFT.
