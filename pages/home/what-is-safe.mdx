...
<Grid container spacing={2} display="flex" alignContent="flex-start" mt={3}>
  <Grid item xs={12} md={6} flex="1">
    <CustomCard
      title={'Safe{Core}'}
      description={
        'The most secure and robust tooling and infrastructure to integrate Safe Smart Account and leverage account abstraction into your product.'
      }
      url={'./safe-core'}
      icon={<SafeCore />}
      newTab={false}
  ...
  <Grid item xs={12} md={6}>
    <CustomCard
      title={'Safe{Wallet}'}
      description={
        'Official interface designed for individuals and industries spanning various sectors, ensuring a seamless and secure digital asset management experience.'
      }
