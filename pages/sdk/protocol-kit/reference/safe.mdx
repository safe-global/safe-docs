...
The `init` method initializes the Protocol Kit instance and connects the Safe account. We have two main ways to initialize the Protocol Kit, depending on whether the Safe account is deployed or not.
...
<Tabs items={["rpc-url.ts", "eip1193.ts"]}>
  <Tabs.Tab>
  ```typescript
  import Safe from '@safe-global/protocol-kit'
...
- The `signer` property can be the signer address, its private key or a passkey object.

This is an example of how to use a passkey object to initialize the SDK:
...
- The `safeAddress` property is the string address of the Safe account. Use this property is the safe is already deployed.

- The `predictedSafe` property is an object that contains the Safe account configuration and deployment details. Use this property if the Safe account has not been deployed yet. The `init` method does not deploy a new Safe; it only enables limited use of the Protocol Kit. To deploy a Safe, you must use the specific deployment methods: `createSafeDeploymentTransaction()` or `executeTransaction()`. This object contains the following properties:
    - `safeAccountConfig`:
...
Returns the Safe address from a Safe deployment transaction receipt. It scans the events emitted during the transaction to identify the [creation event of a Safe account](https://github.com/safe-global/safe-smart-account/blob/main/contracts/proxies/SafeProxyFactory.sol#L12) and returns its address.
