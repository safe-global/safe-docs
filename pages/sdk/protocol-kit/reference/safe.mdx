...
Returns an Ethereum transaction object ready for execution, which includes the deployment of a Safe.
...
#### Getting Started with `createSafeDeploymentTransaction`

Before deploying a new Safe, you must initialize the Protocol Kit with the appropriate configurations. This includes setting the `predictedSafe` property and specifying the setup parameters such as `owners`, `threshold`, and optional parameters like `safeVersion` or `saltNonce`.
...
After the transaction has been executed and the Safe has been successfully deployed, it is necessary to reconnect the Protocol Kit instance to the newly created Safe address by using the `connect` method.
...
// Extract the deployed Safe address from the transaction receipt
...
console.log('Reconnected to the deployed Safe.')
...
#### `getSafeAddressFromDeploymentTx` utility function

The `getSafeAddressFromDeploymentTx` function is an utility that helps extract the Safe address from the transaction receipt of a Safe deployment. This function scans the events emitted during the transaction to identify the [creation event of a Safe](https://github.com/safe-global/safe-smart-account/blob/main/contracts/proxies/SafeProxyFactory.sol#L12) and returns its address.
