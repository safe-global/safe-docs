...
<Steps>
  ...
  // Connect owner3 and the address of safe1_1
  protocolKit = await protocolKit.connect({
    provider: RPC_URL,
    signer: signer3,
    safeAddress: safe1_1
  ...
  // Connect owner4 and the address of safe2_3
  protocolKit = await protocolKit.connect({
    provider: RPC_URL,
    signer: signer4,
    safeAddress: safe2_3
  ...
  ```typescript
  const signerAddress = (await signer1.getAddress()) || '0x'
  ...
  ```typescript
  const signerAddress = (await signer2.getAddress()) || '0x'
