...
- **`signer`** : A passkey (only in the latest alpha `@safe-global/relay-kit@3.1.0-alpha.1` in Sepolia) or the signer private key if the `provider` doesn't resolve to a signer account. If the `provider` resolves to multiple signer addresses, the `signer` property can be used to specify which account to connect, otherwise the first address returned will be used.
