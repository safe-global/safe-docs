...
- The User operations support the usage of a [custom nonce](https://docs.stackup.sh/docs/useroperation-nonce). You can use them by leveraging the `createTransaction` method and passing a `customNonce` in the options parameter. We exported an utility method `encodeNonce` in the `relay-kit` to make it easier to compose the nonce.
...
1. `preEstimateUserOperationGas` : This method, called before using the bundler `eth_estimateUserOperationGas` in the pack code, allows you to adjust the user operation before the bundler estimates it.
2. `postEstimateUserOperationGas` : This method is used after calling `eth_estimateUserOperationGas` in the pack code to adjust the bundler estimation.
