...
In this guide, you will learn how to deploy new Safe accounts and create, sign, and execute Safe transactions using the Safe React Hooks.
...
<Steps>
  ...
  <Tabs items={['New Safe account', 'Existing Safe account']}>
    <Tabs.Tab>
      When deploying a new Safe account for the connected signer, you need to pass the configuration of the Safe in the `safeOptions` property. The Safe account is configured with your signer as the only owner in this case.
  ...
  Once the total number of confirmations reach the `threshold` of the Safe, the `confirmTransaction` will automatically execute the transaction. 
