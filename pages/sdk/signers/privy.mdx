...
<Steps>
  ...
  Once you have it, we need to initialize the `APP_ID` variable with it.
  ...
  In this guide we will use the following variables and methods from the `usePrivy()` and `useWallets()` React hooks.
  ...
  To do that we have a `useEffect()` that is executed when any of the `ready`, `authenticated`, `readyWallets` and `wallets` variables have its value updated. Once they all are `true` and `wallets` has a length greater than zero, we have access to the `wallets` first element, which is the user's connected signer.
...
After following this guide, we are able to create a Safe signer using Privy and get the `provider` and `signerAddress` required to initialize the kits from the Safe\{Core\} SDK.
