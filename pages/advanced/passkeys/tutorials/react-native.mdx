...
| [`@react-native-async-storage/async-storage`](https://github.com/react-native-async-storage/async-storage) | An asynchronous, unencrypted, persistent key-value storage system for React Native. We'll use localStorage in the web and this AsyncStorage on the apps to store the passkey `rawId` and `coordinates`                                           |
...
To facilitate this, we are providing a simple [Node Express web server](https://github.com/5afe/aasa-server) for testing. Fill in your data in the `apple-app-site-association` and `assetlinks.json` files and start the server.
...
The ngrok service will provide you with a public domain. You should copy this domain and add it to your project files:
...
> If you are using iOS simulator remember to add your provisioning profile to sign the app through xcode.
...
After completing all these configurations 🙃, you can finally run the local development build of the app on the emulators by using the following commands:
...
An important consideration is that the selected Android emulator must be compatible with Google Play Services to store the passkeys in the Android keystore. When creating the emulator using Android Studio Device Manager, ensure that you include one with Play Store services.

![react-native-passkeys-app-2.png](../../../../assets/react-native-passkeys-play-store.png)

After building and initializing, you should see the app running on the emulator. You can start developing the app.
...
We are adding 2 functions here:

- `createPasskey` helps generate a new passkey using `react-native-passkeys`.
- `getPasskey` retrieves the passkey using the `react-native-passkeys` library. This method is necessary for pass to the [protocol-kit init](TODO: document this and add url) method and will be used to sign any message or transaction.
