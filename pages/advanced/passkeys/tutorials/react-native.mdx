...
We are including 4 new utilities to deploy a safe, sign messages, and sign and execute transactions.

- `activateAccount` will deploy a new Safe account on Sepolia based on the predicted Safe configuration.
- `addPasskeyOwner` adds a new passkey owner to the Safe account.
- `signPasskeyMessage` assists in signing messages using the passkey signer.
...
Using the existing `App.tsx` file, we will add the UI components to interact with the Safe account and passkeys. Let's update the whole content and use this instead:

```tsx
// from ../../../../examples/react-native-passkeys/components/App.tsx
```

Now we should have a working app that allows the user to activate the Safe account and interact with passkeys.
