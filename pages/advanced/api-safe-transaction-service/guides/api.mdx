...
This guide shows how to interact with the Safe Transaction Service API in different situations.

The examples have been implemented using [Curl](https://github.com/curl/curl) requests, the [Safe{Core} SDK](https://github.com/safe-global/safe-core-sdk) TypeScript library and the [safe-eth-py](https://github.com/safe-global/safe-eth-py) Python library.

## Prerequisites

1. [Node.js and npm](https://docs.npmjs.com/downloading-and-installing-node-js-and-npm#using-a-node-version-manager-to-install-nodejs-and-npm) if using the Safe{Core} SDK.
2. [Python](https://www.python.org/downloads/) >= 3.9 if using `safe-eth-py`.
3. Have a Safe account configured with threshold equal to two.
...
<Steps>
  ...
  <Tabs items={['TypeScript', 'Python', 'Curl']}>
    <Tabs.Tab>
      ...
      // Add Owner B as delegate of Owner A for all Safes accounts (safeAddress = null)
