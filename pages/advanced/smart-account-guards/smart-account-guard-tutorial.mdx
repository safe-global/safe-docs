...
You will build a `NoDelegatecallGuard` that blocks `delegatecall` through Safe account when executing a Safe Transaction.
...
Explanation:
- `BaseGuard` is inherited to implement the Guard interface. The Guard interface has two functions:
