...
### Limitations
- The nonce in the contract increases sequentially, meaning tokens must be withdrawn one at a time and in the correct order. Each withdrawal depends on completing the previous one.
...
Delete the default `contracts/Lock.sol` and test file `test/Lock.ts` and create a new Solidity contract `TokenTransferModule.sol` in the `contracts` directory.
...
Create a new file named `TokenTransferModule.test.ts` and include the following basic structure:

```typescript
// Import necessary libraries and types

describe("TokenTransferModule Tests", function () {
...
#### Step 2: Create test/utils/utils.ts file
...
This setup ensures that each test starts with a fresh instance of the contracts and the necessary initial state.
