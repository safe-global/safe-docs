...
Finding the address of the new Singleton contract is the first step in the migration process. Safe Team maintains [SafeMigration](https://github.com/safe-global/safe-smart-account/blob/main/contracts/libraries/SafeMigration.sol) contract to update the Singleton contract address in the `SafeProxy` contract.
...
- The migration script can be extended to support Safe Account migration with threshold more than one. The users can use [Safe API Kit](https://github.com/safe-global/safe-core-sdk/tree/main/packages/api-kit) to propose the transaction, fetch transaction data and sign the transaction.
- Source code of this script is available in the [Safe Migration Script repository](https://github.com/5afe/safe-migration-script).