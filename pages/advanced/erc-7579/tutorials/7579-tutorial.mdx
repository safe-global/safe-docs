...
<Callout type='warning' emoji='⚠️'>
  ...
  accounts, which is generally considered insecure. Multi-signature ERC-7579 Safes will come soon.
</Callout>
...
If the module is installed, this component will create the [`UserOperation`](/glossary#useroperation) to recover the Safe.

Then, create a new file `Guardian.tsx` in the `components` folder:
...
This component will provide state for each guardian that will be added, and will allow them to sign a hash of this `UserOperation`. It will request access to the user wallet (such as Metamask for example), and prepare the message to sign.
