...
Let's say you want to build an app to enable scheduling transfers for monthly salaries to a team of contributors. However, Safe does not offer a native module for scheduling transfers. With the ERC-7579 compatibility, you can use Rhinestone's [Social Recovery module](https://docs.rhinestone.wtf/module-sdk/modules/social-recovery) with Safe to build an app to recover your Safe in case you lose access to it.
...
<Callout type='warning' emoji='⚠️'>
  ...
  accounts. Multi-signature ERC-7579 Safes will be coming soon.
</Callout>
...
Add the code necessary to create Pimlico's client by adding this content to `permissionless.ts` :
...
- Pass the Safe Smart Account object to `getPermissionlessAccount` to generate a Permissionless client.
...
- `addGuardian` will add a guardian to the Safe.
- `getGuardians` to get all the guardians added in a Safe where the module has been previously installed.
- `recoverSafe` to recover the Safe by adding a guardian as a new owner.

In the UI, we can then detect whether the Safe has the module installed. If not, it will let the user add guardians before installing it.
...
This file will contain functions to get useful Safe data via the Protocol Kit:
...
- `deploySafe` will deploy a Safe (if it is not already deployed), here simply by making the Permissionless client send a dummy transaction.

We will also add a file `utils.ts` in the `lib` folder, to contain a formatting function that will be used to display addresses in a more readable format.:
...
This component will provide a form to allow the user to input the address of `n` guardians, where `n` is a threshold that can be modified in the code. The component will detect whether the Safe has the module installed, and then call `install7579Module`.

If the module is installed, this component will create the [`UserOperation`](/glossary#useroperation) to recover the Safe. The component will allow all the guardians to sign a hash of this `UserOperation`. It will request access to the user wallet (such as Metamask for example), and prepare the message to sign.
