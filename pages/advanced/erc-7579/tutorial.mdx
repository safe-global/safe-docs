# **How to build an app with Safe and 7579**
...
[ERC-7579](https://docs.safe.global/advanced/erc-7579/overview) aims to ensure interoperability across implementations. It defines the account interface so developers can implement modules for all smart accounts that follow this standard. The Safe7579 Adapter makes your Safe compatible with any 7579 modules. As an application developer building with Safe, you get access to a rich ecosystem of modules to make your application feature-rich.

Let's say you want to build an app to enable scheduling transfers for monthly salaries to a team of contributors. However, Safe does not offer a native module for scheduling transfers. With the 7579 compatibility, you can use Rhinestone's [Scheduled Transfer module](https://github.com/rhinestonewtf/modulekit/blob/examples/audit-ready/examples/src/ScheduledTransfers/ScheduledTransfers.sol) with Safe to build an app to schedule transfers ahead of time.

This tutorial will teach you to build an app that can:

- Deploy a 7579-enabled Safe.
...
**Prerequisite knowledge:** You will need some basic experience with [React](https://react.dev/learn), [Next.js](https://nextjs.org/docs), [ERC-4337](https://docs.safe.global/home/4337-overview) and ERC-7579.
...
For this project, we'll use Pimlico's [Permissionless.js](https://docs.pimlico.io/permissionless) to set up a Safe, Rhinestone's [Module SDK](https://docs.rhinestone.wtf/module-sdk) for ERC-7579 functionalities, and [viem](https://www.npmjs.com/package/viem) for some helper functions.

Run the following command to add all these dependencies to the project:

```shell
pnpm add permissionless@0.1.34 viem @rhinestone/module-sdk
```
...
!https://passkeys--docs.review.5afe.dev/_next/static/media/next.9a3408db.png

## 2. Initialize `permissionless` client

Create a `lib` folder at project root, and add a file `permissionless.ts`:
...
```typescript
// from ./examples/lib/permissionless.ts
```

It will load a `PRIVATE_KEY` from `.env.local` (or generate one if it doesn’t exist) and then create the `publicClient`, `paymasterClient` and `bundlerClient` necessary to initialize the `permissionless` library. It then creates a 7579-enabled Safe Smart Account from the generated private key, before passing it to `createSmartAccountClient` to generate a Permissionless client.
...
```typescript
// from ./examples/lib/ScheduledTransfers.ts
```

This file contains two functions:
- `install7579Module` will install the module to a Safe and schedule its first transfer; 
- `scheduleTransfer` to schedule transfers in a Safe where the module has been previously installed.
...
```tsx
// from ./examples/components/ScheduledTransferForm.tsx
```

This component will detect whether the Safe has the module installed, and allow the user to input the amount, receiver address, and date and time for the scheduled transfer.

Now, edit `app/page.tsx` to include the `ScheduledTransferForm` component:

```tsx
// from ./examples/app/page.tsx
```
...
![screenshot-finished](../../../assets/7579-tutorial-2.png)

Click the **Load Smart Account Client** to initialize Permissionless' client with the private key you stored on `.env.local`. It will deploy an ERC-7579-enabled Safe on its first transaction.

Once loaded, you will be able to chose an amount to send, a receiver address and select a date and time for your scheduled payment. Click **Schedule Transfer** to send the transaction. The first time you do this, it will deploy the Safe to Sepolia test network and install the ERC-7579 compatible ScheduledTransfers module.

## **Do more with Safe and 7579**

Today, we learned how to deploy a 7579-enabled Safe and use a 7579-compatible module, the Scheduled Transfer from Rhinestone. We hope you enjoyed this tutorial and that the combination of Safe and 7579 will allow you to tap into new functionalities for your decentralized apps.

As a next step, you can add more functionalities to your app using other [7579-compatible modules](https://docs.rhinestone.wtf/module-sdk/modules/ownable-validator). Here are some ideas:
...
- [Designate an owner](https://docs.rhinestone.wtf/module-sdk/modules/ownable-executor) that can act on your behalf for executing transactions and pay for gas.

You can also find more inspiration on this list of [awesome 7579 modules](https://github.com/rhinestonewtf/awesome-modular-accounts?tab=readme-ov-file#modules). You can also read more about this ERC in our [overview](https://docs.safe.global/advanced/erc-7579/overview) or in the [official documentation](https://erc7579.com/).
