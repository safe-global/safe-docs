...
<Callout type="info" emoji="ℹ️">
  You must first load the sender's private key. When loading an owner, it will
...
<Callout type="info" emoji="ℹ️">
  Before signing anything, ensure that the data on your hardware wallet device
  is the same as the Safe CLI data.
</Callout>
...
<Callout type="warning" emoji="⚠️">
  Only use the following operations if you are sure about what you are doing, as
  they can result in all your funds getting lost.
</Callout>
...
<Callout type="warning" emoji="⚠️">
  A non-L2 Safe can only be migrated to L2 if the non-L2 Safe was not used
  before (nonce must be zero).
</Callout>
