...
To recreate the Safe, it is necessary to execute `safe-creator` as follows:
...
If you've recreated a Safe from an L1 network (like mainnet) on an L2 network, our services cannot index them. That's because for L1, we use trace-based indexing, and for L2 events indexing, L1 Safe singleton does not emit events.
...
For detailed instructions on running these commands, please refer to the [common operations](../cli-reference.mdx#common-operations) section for more information.