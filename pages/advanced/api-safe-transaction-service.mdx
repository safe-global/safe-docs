...
In the [Safe Transaction Service](https://github.com/safe-global/safe-transaction-service) repository under [Setup for production (event indexing)](https://github.com/safe-global/safe-transaction-service?tab=readme-ov-file#setup-for-production-event-indexing) and [Setup for production (tracing mode)](https://github.com/safe-global/safe-transaction-service?tab=readme-ov-file#setup-for-production-tracing-mode) show how to run the service and retrieve events from any supported network type. Note that the configuration must be adapted to the needs of a specific use case.

In addition, [Service maintenance](https://github.com/safe-global/safe-transaction-service?tab=readme-ov-file#service-maintenance) shows how to deal with some common errors in the service operation.