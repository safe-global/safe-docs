...
export const dedupeReducer = (acc: DocContent[], curr: DocContent) => {
...
export function findFunctionNameInFile(file: string, functionName: string) {
  try {
    const fileContent = fs.readFileSync(file, 'utf8')
...
export const walkPath = (dir: string) => {
  let results: string[] = []
  try {
